...
class ValueNetwork(nn.Module):
    def __init__(self, state_dim, hidden_dim, with_layer_norm=False, init_w=3e-3):
    ...
    def forward(self, state):
        if self.with_layer_norm:
            x = F.relu(self.layer_norm1(self.linear1(state)))
...
class SoftQNetwork(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_size, with_layer_norm=False, init_w=3e-3):
    ...
    def reset(self, **kwargs):
        ...
        with self.env.sim.no_rendering():
            if whether_random:
                ...
                ee_pos = kwargs.get('ee_pos', None)
                if ee_pos is not None:
                    target_arm_angles = self.env.robot.inverse_kinematics(
                        link=self.env.robot.ee_link, position=ee_pos, orientation=np.array([1.0, 0.0, 0.0, 0.0])
                    )
                    target_arm_angles[-1] = 0.0
                    target_arm_angles[-2] = 0.0
                    self.env.robot.set_joint_angles(target_arm_angles)
