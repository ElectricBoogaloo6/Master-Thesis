...
def mlp(sizes, activation, output_activation=nn.Identity, type='pi', with_layer_norm=False):
    layers = []
    for j in range(len(sizes) - 1):
        act = activation if j < len(sizes) - 2 else output_activation

        if with_layer_norm and type == 'critic':
            if j < len(sizes) - 2:
...
class MLPVFunction(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, hidden_sizes, activation, with_layer_norm=False):
        super().__init__()
        self.v = mlp([obs_dim] + list(hidden_sizes) + [1], activation, type='critic', with_layer_norm=with_layer_norm)
...
class MLPQFunction(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, hidden_sizes, activation, with_layer_norm=False):
        super().__init__()
        self.q = mlp([obs_dim + act_dim] + list(hidden_sizes) + [1], activation, type='critic', with_layer_norm=with_layer_norm)
...
class MLPActorCritic(nn.Module):
    ...
    def __init__(self, observation_dim, action_space, device, with_layer_norm=False, hidden_sizes=(256, 256),
        ...
        self.q1 = MLPQFunction(obs_dim, act_dim, hidden_sizes, activation, with_layer_norm=with_layer_norm).to(device)
        self.q2 = MLPQFunction(obs_dim, act_dim, hidden_sizes, activation, with_layer_norm=with_layer_norm).to(device)
        self.v = MLPVFunction(obs_dim, act_dim, hidden_sizes, activation, with_layer_norm=with_layer_norm).to(device)
