...
class Phase1TaskRight(Task):
    def __init__(
        ...
        # KM: goal position is set to the right of the table
        self.goal_position = np.array([-0.30, 0.10, self.object_size / 2.0])
        ...
        # KM: predefined starting position for the cube in environment (in a line)
            # changes can be on y axis while x and z stay the same. goes by [x, y, z]
                # x = -0.15 because it should be along the same line on x coordinate. z = object_size / 2.0 because obj should be correct height above the ground (half the object size to centre it on the ground)
                # y = increments to spread accross the y-axis in a range. 
        ...
        # print(f"Predefined object positions: {self.predefined_obj_positions}")
    ...
    def _sample_goal(self) -> np.ndarray:
        """Randomize goal."""
        goal = np.array([0.15, 0.0, self.object_size / 2.0])
